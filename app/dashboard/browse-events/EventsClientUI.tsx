...
import { Search, MapPin, Calendar, XCircle, Frown, ArrowRight, RefreshCw, Share2, Check, Users, UserCheck, UserX, Filter, ChevronDown, ChevronUp, Clock  } from 'lucide-react';
...
const EventsClientUI = () => {
  ...
  const [teamOnly, setTeamOnly] = useState(searchParams.get('teamOnly') === 'true' || false);
 ...
 const updateUrlParams = useCallback((
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateUrlParams(value, currentLocation, currentDate, currentSlots, slotStatus, teamOnly);
  };

  const handleLocationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setCurrentLocation(value);
    updateUrlParams(currentSearch, value, currentDate, currentSlots, slotStatus, teamOnly);
  };

  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setCurrentDate(value);
    updateUrlParams(currentSearch, currentLocation, value, currentSlots, slotStatus, teamOnly);
  };

  const handleSlotsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setCurrentSlots(value);
    updateUrlParams(currentSearch, currentLocation, currentDate, value, slotStatus, teamOnly);
  };

  const handleSlotStatusChange = (status: string) => {
    setSlotStatus(status);
    updateUrlParams(currentSearch, currentLocation, currentDate, currentSlots, status, teamOnly);
  };

  const clearSearch = () => {
    setCurrentSearch('');
    updateUrlParams('', currentLocation, currentDate, currentSlots, slotStatus, teamOnly);
  };

  const clearLocation = () => {
    setCurrentLocation('');
    updateUrlParams(currentSearch, '', currentDate, currentSlots, slotStatus, teamOnly);
  };

  const clearDate = () => {
    setCurrentDate('');
    updateUrlParams(currentSearch, currentLocation, '', currentSlots, slotStatus, teamOnly);
  };

  const clearSlots = () => {
    setCurrentSlots('');
    updateUrlParams(currentSearch, currentLocation, currentDate, '', slotStatus, teamOnly);
  };
...
const resetAllFilters = () => {
  ...
  const renderEventAvailability = (event: Event) => {
    if (event.teamOnly || event.allowFreePlayersIfTeamIncomplete) {
      ...
      return (
        <div className="flex flex-col space-y-1">
          <div className="flex items-center text-xs text-green-600">
            <Users className="h-3 w-3 mr-1" />
            <span>Players: {event.participantsCount}/{event.slots}</span>
          </div>
          <div className="flex items-center text-xs text-blue-600">
            <Users className="h-3 w-3 mr-1" />
            <span>Teams: {event.teamCount}/2</span>
      ...
      return (
        <div className={`flex items-center text-xs ${event.availableSlots === 0 ? 'text-red-500' : 'text-green-600'}`}>
          <Users className="h-3 w-3 mr-1" />
          <span>Available: {event.availableSlots}/{event.slots}</span>
          {!event.isAvailable && (
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="hidden lg:block w-72 flex-shrink-0">
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-6 transition-all duration-300 hover:shadow-lg">
              <div className="flex justify-between items-center mb-4 cursor-pointer" onClick={() => setIsFilterSectionOpen(!isFilterSectionOpen)}>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md text-sm flex items-center transition-colors ${slotStatus === 'all' ? 'bg-indigo-100 text-indigo-800' : 'hover:bg-gray-100'}`}
                      >
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md text-sm flex items-center transition-colors ${slotStatus === 'available' ? 'bg-green-100 text-green-800' : 'hover:bg-gray-100'}`}
                      >
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md text-sm flex items-center transition-colors ${slotStatus === 'unavailable' ? 'bg-red-100 text-red-800' : 'hover:bg-gray-100'}`}
                      >
...
<div>
  ...
  <div className="space-y-2">
    <button
      onClick={() => {
        setTeamOnly(false);
        updateUrlParams(currentSearch, currentLocation, currentDate, currentSlots, slotStatus, false);
      }}
      className={`w-full text-left px-3 py-2 rounded-md text-sm flex items-center transition-colors ${!teamOnly ? 'bg-indigo-100 text-indigo-800' : 'hover:bg-gray-100'}`}
    >
      <Users className="w-4 h-4 mr-2" />
      All Event Types
    </button>
    <button
      onClick={() => {
        setTeamOnly(true);
        updateUrlParams(currentSearch, currentLocation, currentDate, currentSlots, slotStatus, true);
      }}
      className={`w-full text-left px-3 py-2 rounded-md text-sm flex items-center transition-colors ${teamOnly ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
    >
      <Users className="w-4 h-4 mr-2" />
      Team Events Only
    </button>
          ...
          {/* Mobile Filters */}
          {isMobileFiltersOpen && (
            <div className="lg:hidden fixed inset-0 z-50 overflow-y-auto bg-gray-500 bg-opacity-75 transition-opacity">
              <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div className="fixed inset-0 transition-opacity" onClick={() => setIsMobileFiltersOpen(false)}>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <button
                            onClick={() => handleSlotStatusChange('all')}
                            className={`px-3 py-2 rounded-md text-sm flex items-center justify-center transition-colors ${slotStatus === 'all' ? 'bg-indigo-100 text-indigo-800 border border-indigo-200' : 'border border-gray-200 hover:bg-gray-50'}`}
                          >
                          ...
                          <button
                            onClick={() => handleSlotStatusChange('available')}
                            className={`px-3 py-2 rounded-md text-sm flex items-center justify-center transition-colors ${slotStatus === 'available' ? 'bg-green-100 text-green-800 border border-green-200' : 'border border-gray-200 hover:bg-gray-50'}`}
                          >
                          ...
                          <button
                            onClick={() => handleSlotStatusChange('unavailable')}
                            className={`px-3 py-2 rounded-md text-sm flex items-center justify-center transition-colors ${slotStatus === 'unavailable' ? 'bg-red-100 text-red-800 border border-red-200' : 'border border-gray-200 hover:bg-gray-50'}`}
                          >
...
<div>
  ...
  <div className="grid grid-cols-2 gap-2">
    <button
      ...
      className={`px-3 py-2 rounded-md text-sm flex items-center justify-center transition-colors ${!teamOnly ? 'bg-indigo-100 text-indigo-800 border border-indigo-200' : 'border border-gray-200 hover:bg-gray-50'}`}
    >
      <Users className="w-4 h-4 mr-1" />
      All Types
    </button>
    <button
      ...
      className={`px-3 py-2 rounded-md text-sm flex items-center justify-center transition-colors ${teamOnly ? 'bg-blue-100 text-blue-800 border border-blue-200' : 'border border-gray-200 hover:bg-gray-50'}`}
    >
      <Users className="w-4 h-4 mr-1" />
      Team Only
    </button>
...
>
  ...
  <div className="absolute top-3 right-3 flex flex-col items-end gap-2">
    ...
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusDisplay(event.status).color}`}>
  ...
  className={`inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white transition-colors ${
    event.isAvailable && event.status === 'not_started'
      ? event.teamOnly || event.allowFreePlayersIfTeamIncomplete
        ? 'bg-blue-600 hover:bg-blue-700'
        : 'bg-indigo-600 hover:bg-indigo-700'
      : 'bg-gray-400 cursor-not-allowed'
  }`}
  disabled={event.status === 'ended' || event.status === 'ongoing'}
>
  {event.status === 'ended' 
    ? 'Event Ended' 
    : event.status === 'ongoing'
      ...
      {selectedEvent && !selectedEvent.teamOnly && !selectedEvent.allowFreePlayersIfTeamIncomplete && (
        <EventDrawer
          event={selectedEvent}
